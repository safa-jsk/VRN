...
from src.vrn.utils import ensure_ext_importable

# Ensure CUDA extension is importable from artifacts/build or external/
ensure_ext_importable()
...
try:
    ...
    try:
        import marching_cubes_cuda_ext  # noqa: F401  (built .so on sys.path)
        from external.marching_cubes_cuda_ext.cuda_marching_cubes import marching_cubes_gpu
        USE_CUSTOM_CUDA = True
        print("✓ Using custom CUDA marching cubes kernel (via built extension)")
